...
const UserController_1 = __importDefault(require("../Mongoose/UserController"));
const RoomController_1 = __importDefault(require("../Mongoose/RoomController"));
const MessageController_1 = __importDefault(require("../Mongoose/MessageController"));
function socketIO(socketServer) {
    socketServer.on('connection', (socket) => {
        console.log('클라이언트와 연결이 성공했습니다.', 'socket.id:', socket.id);
        //앱이 실행될 때 DB에 3개의 방 생성
        socket.on('createRooms', async (cb) => {
            const newRooms = await RoomController_1.default.createRooms();
            if (newRooms.length) {
        ...
        socket.on('welcome', async (name, cb) => {
            const newUser = await UserController_1.default.createUser(name, socket.id);
            if (newUser) {
        ...
        socket.on('addMemberToRoom', async (roomId, userName) => {
            ...
            if (socket.rooms.has(roomId))
                return;
            const addedMemberRoom = await RoomController_1.default.addMember(roomId, userName);
            //멤버가 정상적으로 룸에 등록됐다면
            console.log(2);
            if (addedMemberRoom) {
                console.log(3);
                socket.join(roomId);
                const user = await UserController_1.default.findUser(userName);
                console.log(4);
                if (user) {
                    const welcomeMessage = await MessageController_1.default.addMessage('welcome', '', roomId, userName);
                    console.log(5);
                    if (welcomeMessage) {
                        console.log(6);
        ...
        socket.on('message', async (message, roomId, userName) => {
            const newMessage = await MessageController_1.default.addMessage('normal', message, roomId, userName);
            const populatedMsg = await newMessage?.populate('author');
            socketServer.to(roomId).emit('updateMessage', populatedMsg);
        });
        socket.on('exitRoom', async (roomId, userName, cb) => {
            socket.leave(roomId);
            const removedMemberRoom = await RoomController_1.default.removeMember(roomId, userName);
            const byeMessage = await MessageController_1.default.addMessage('leave', '', roomId, userName);
        ...
        socket.on('disconnecting', async (reason) => {
            ...
            const roomPromises = Array.from(socket.rooms).map(async (roomId) => {
                const byeMessage = await MessageController_1.default.addMessage('leave', '', roomId, socket.id);
                const removedMemberRoom = await RoomController_1.default.removeMember(roomId, socket.id);
                console.log('새로고침할떄', 'byeMessage:', byeMessage, 'romvedMeberRoom:', removedMemberRoom);
                socket.to(roomId).emit('updateMessage', byeMessage);
                socket.to(roomId).emit('updateRoom', removedMemberRoom);
            });
            await Promise.all(roomPromises);
            await UserController_1.default.deleteUser(socket.id);
        });
    });
}
